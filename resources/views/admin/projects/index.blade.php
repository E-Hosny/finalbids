@php
    ...
    $auction = \App\Models\Auctiontype::where('status',1)->get();
@endphp


<x-admin-layout>
    <main class="main-content position-relative border-radius-lg ">
        @include('admin.include.navbar', ['module' => 'Projects', 'link' => 'admin.projects.index', 'page' => ''])
        ...
        @include('flash-message')
         <div class="row">
           <!-- <div class="col-lg-2 col-md-6">
            ...
            <!-- <div class="col-lg-2 col-md-6">
                <label class="tests" for="langId"><strong>Select Auction Type:</strong></label>
                <select name="lang_id" class="choices__list choices__list--single form-control" id="langId" tabindex="-1" data-choice="active">
                @foreach ($auction as $act)
                    <option value="{{ $act->short_name }}">{{ $act->name }}</option>
                @endforeach
                </select>
            </div> -->
...
$(document).ready(function() {
    if ($.fn.dataTable.isDataTable('#project-table')) {
        var data = $('#project-table').DataTable();
    } else {
        var data = $('#project-table').DataTable({
            paging: false
        });
