...
class BidPlacedDataTable extends DataTable
{
    ...
    //             return $row->user ? $row->user->first_name : 'N/A'; // عرض اسم المستخدم
    //         })
    //         ->addColumn('project', function ($row) {
    //             return $row->project ? $row->project->name : 'N/A'; // عرض اسم المشروع
    //         })
    //         ->addColumn('product', function ($row) {
    //             return $row->product ? $row->product->title : 'N/A'; // عرض اسم المنتج
    //         })
    ...
    //             return $row->created_at->format('Y-m-d H:i:s'); // تنسيق التاريخ
    //         })
    //         ->rawColumns(['status']) // السماح بـ HTML في عمود الحالة
    //         ->addColumn('action', 'admin.bid_placed.status') // عرض الأزرار
    //         ->setRowId('id');
    // }
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
{
    ...
    // {
    //     return $model->with(['user', 'project', 'product', 'auctiontype'])->newQuery();
    // }
    public function query(BidPlaced $model): QueryBuilder
{
